...
#This version of the script takes two VCFs as input rather than
# merging. The first VCF is that with the archaics and ancestral
# sequence, and these alleles are read into memory rather than
# being streamed in from a single merged VCF.
#
#Options/arguments:
...
#Second file is the archaic and ancestral VCF
filenum==2&&/^#CHROM/{
...
filenum==2&&!/^#/{
   ...
   for (i=10; i<=NF; i++) {
      ...
      for (a in samplealleles) {
         targetallelemap[$1,$2,sampleregion,a]++;
         if (length(debug) > 0) {
...
#Third file is the modern population VCF
...
filenum==3&&!/^#/{
   ...
   excludeAN=0;
   #By pre-sorting so that excludes come first, then the rest,
   ...
   for (i=1; i<=n_indivs; i++) {
      ...
      if (sampleregion in targets) {
         print "Target individual "ids[indivorder[i]]" found in modern VCF. This isn't allowed for this version of the script. Quitting." > "/dev/stderr";
         exit 3;
      };
      split($indivorder[i], sample, ":");
      split(sample[1], gt, "[/|]");
      #Skip the individual if it's any query population and we aren't
      # coercing missing genotypes to homozygous reference.
...
#            };
            ...
            for (j=1; j<=n_targets; j++) {
               if (($1,$2,targetarr[j],a) in targetallelemap) {
